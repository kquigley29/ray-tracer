...
{
    Eigen::Vector3d hit(std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity());
    ...
    for (auto obj : this->objects)
    {
        ...
        {
            double temp_dist1 = (intersection.hit_point - this->camera.get_position()).dot(intersection.hit_point - this->camera.get_position());
            ...
            if (temp_dist1 < temp_dist2)
            {
                ...
                for (auto light : this->lights)
                {
                    ...
                    bool is_in_shadow = this->get_hit(intersection.hit_point + (0.000001 * (light->get_position() - intersection.hit_point)), light->get_position());
...
void Scene::generate_scene(char *filename)
{
    std::vector<std::vector<std::string>> parts = read_cfg(filename);
    int count = 0;
    for (const auto &options: parts)
    {
        if (options[0] == "[camera]")
        {
            this->add_camera(Camera::parse(options));
        }
        if (options[0] == "[sphere]")
        {
            this->add_object(Sphere::parse(options));
            count++;
        }
        if (options[0] == "[plane]")
        {
            this->add_object(Plane::parse(options));
            count++;
        }
        if (options[0] == "[point_light]")
        {
            this->add_light(PointLight::parse(options));
            count++;
        }
    }
    printf("Scene generated with %d objects\n", count);
}
